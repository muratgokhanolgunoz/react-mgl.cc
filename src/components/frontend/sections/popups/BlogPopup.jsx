...
import elementsJSON from "../../../../utils/html_elements/elements.json";
...
class BlogPopup extends Component {
    ...
    parseTextToHMTL = () => {
        if (this.props.popupShow) {
            ...
            this.showArticle(
                this.props.blogDetails.BLOG_SECTION_ITEMS_ARTICLE,
                this.sortArray(arrayFindElements)
            );
        }
    };
        ...
        if (_elements.length > 0) {
            for (let i = 0; i < _elements.length; i++) {
                ...
                if (_elements[i].tag.isContent === true) {
                    createElement.innerHTML = this.getArticleText(
                        _article,
    ...
    render() {
        return (
            <div>
                <Modal
                    show={this.props.popupShow}
                    fullscreen={true}
                    onHide={() => this.props.popupShowToggle(false)}
                    animation={false}
                    size="lg"
                    backdrop={true}
                >
                    <Modal.Header>
                        <Button
                            className="template-button template-button-danger pin-to-right"
                            onClick={() => this.props.popupShowToggle(false)}
                        >
                    ...
                    <Modal.Body>
                        <Container>
                            <Row>
                                <Col
                                    className="modal-blog-preview-body"
                                    lg={{ span: 8, offset: 2 }}
                                    md={12}
                                >
                                    <Row>
                                        <Image
                                            className="modal-blog-preview-body-image"
                                            src={this.props.blogDetails.BLOG_SECTION_ITEMS_PHOTO}
                                            alt=""
                                            fluid
                                        />
                                        <Image
                                            className="modal-blog-preview-body-thumbnail"
                                            src={
                                                this.props.blogDetails.BLOG_SECTION_ITEMS_THUMBNAIL
                                            }
                                            alt=""
                                            fluid
                                        />
                                    ...
                                    <Row>
                                            ...
                                            <p style={{ textAlign: "right" }}>
                                                <small>
                                                    {this.props.blogDetails
                                                        .BLOG_SECTION_ITEMS_DATE !== undefined
                                                        ? this.props.blogDetails.BLOG_SECTION_ITEMS_DATE.substr(
                                                              0,
                                                              10
                                                          )
                                                        : null}{" "}
                                        ...
                                        <Col md={12} style={{ marginTop: "-40px" }}>
                                            <h1>
                                                {this.props.blogDetails.BLOG_SECTION_ITEMS_TITLE}
                                            </h1>
                                        </Col>
                                    </Row>
                                    <Row>
